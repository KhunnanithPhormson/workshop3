...
function customizeSelect(self, attribuites) {
  ...
  if (typeof attribuites.display == "undefined")
    attribuites.display = "table-cell";
  ...
  var selectContent =
    $this.find("[selected]").length == 0
      ? $this.children("option").eq(0).html()
      : $this.find("[selected]").html();
  $styledSelect.html(
    (attribuites.filter
      ? '<input class="input-item" type="text"/> <div class="input-text"'
      : "<div ") +
    'style="height: ' +
    attribuites.height +
    ";vertical-align: " +
  ...
  $styledSelect.click(function (e) {
    $(".selectlang").clear_anim();
    if ($(this).find("input").is(":visible") && attribuites.filter)
      return false;
    if (!isSelectDisabled) {
      ...
      .each(function (index, elem) {
        if (
          !$(elem)
            .text()
            .toLowerCase()
            .startsWith($(event.target).val().toLowerCase(), 0)
        ) {
          $(elem).hide();
        } else {
          $(elem).show();
